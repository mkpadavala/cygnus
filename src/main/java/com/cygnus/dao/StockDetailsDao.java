...
import com.cygnus.util.StockUtil;

public class StockDetailsDao {

	private static final String GET_TICKERS = "SELECT TICKER FROM test_TICKERS";
	...
	private static String SCRUBBED_INSERT_QUERY= "INSERT INTO TEST_STOCK_DETAILS VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	
	private static String MAKET_GAINERS = "select * from test_stock_details where TRADEDATE = ? and gainorloss > 0" 
											+ " and marketcap > ? and totalvolume > ? order by";
	
	private static String MARKET_LOOSERS = "select * from test_stock_details where TRADEDATE = ? and gainorloss < 0"
											+ " and marketcap > ? and totalvolume > ? order by";
	...
	private static List<Stock> convertToStockList(ResultSet rs, int howmany)throws Exception{
		...
		if(rs != null){
			...
			while (rs.next()) {
				...
				stock.setCreationDate(StockUtil.getDateFormatter().parse(rs.getString(CREATION_DATE)));
	...
	public static List<Stock> getStockDetailsFromDB()throws Exception{
		...
		try{
			...
			ResultSet rs = psmt.executeQuery();
			stocks = convertToStockList(rs, -1);			
